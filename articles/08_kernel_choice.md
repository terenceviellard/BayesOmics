# 08 · Using a kernel other than Squared Exponential
...
The SE kernel assumes very smooth correlation. Nothing else in the
pipeline depends on that choice – swapping the constructor is enough.

| Kernel | Use when |
|----|----|
| [`matern12_kernel()`](https://rdrr.io/pkg/keRnel/man/matern_kernels.html) | Rough, non-smooth correlation |
| [`matern32_kernel()`](https://rdrr.io/pkg/keRnel/man/matern_kernels.html), [`matern52_kernel()`](https://rdrr.io/pkg/keRnel/man/matern_kernels.html) | Intermediate smoothness, more realistic than SE for noisy biological signal |
| `periodic_kernel(length_scale, period)` | Cyclical structure (e.g. circadian, seasonal) |
| [`linear_kernel()`](https://rdrr.io/pkg/keRnel/man/linear_kernel.html) | A trend rather than a bump/dip |

Two worked examples below, each with the kernel’s
correlation-vs-distance shape next to
[`keRnel::plot_kernel()`](https://rdrr.io/pkg/keRnel/man/plot_kernel.html)’s
heatmap of the same kernel over the actual feature positions – the 1D
curve on the left is exactly what the 2D banding on the right is built
from.
...
`shape(kernel, distance_function(x1, x2))`, so this is the same
correlation a heatmap cell would show, just as a 1D curve over distance
alone:
...
![](08_kernel_choice_files/figure-html/unnamed-chunk-3-1.png)
...
![](08_kernel_choice_files/figure-html/unnamed-chunk-5-1.png)
...
shape’s oscillation on the left – an SE or Matern kernel could not
produce this pattern regardless of its length scale.
...
- The kernel constructor is the only thing that changes between kernel
  ...
  and every downstream metric are constructor-agnostic.
...
- Every kernel here still benefits from a `+ white_noise_kernel(...)`
  term for the same numerical-stability reason as the SE case.

## Related

[Basic
pipeline](https://terenceviellard.github.io/BayesOmics/articles/01_basic_pipeline.md)
– [Input with more than one
